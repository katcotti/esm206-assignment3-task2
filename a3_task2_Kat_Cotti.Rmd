...
```{r}
# Your code here: 
sw_split_name <- starwars %>%  #Start with starwars data set 
  separate(col = name, #slits the name column 
           into = c("name_1", "name_2"), #into two pieces 'name_1' and 'name_2'
           sep = '-') #separates at the first dash 
...
```{r}
# Your code here:
sw_skin_sep <- starwars %>% #create new data set starting with the starwars data
  separate(col = skin_color, #separate skin color into 2 columns
           into = c("skin_1", "skin_2"), 
           sep = ",", #separate at the first comma
           extra = "merge", #merge everything after the first common into the second column
           remove = FALSE) #keep the original skin color column 
...
```{r}
# Your code here: 
mpg_model_class <- mpg %>% #make new data frame from mpg data set
  unite(col = model_class, #make a new column
        c(model, class), #combining model and class 
        sep = "-") #separate by a dash 
...
```{r}
# Your code here: 
gasol_hookshots <- lakers %>%  #Create new data frame from lakers data set
  filter(player == "Pau Gasol", type == "hook") %>% #Filter to only include Pau Gasol's hookshots
  mutate(date_new = ymd(date)) %>% #update date column to ymd
  mutate(month_new =month(date_new)) #Add a new colum that contains the month

hooks_by_month <- gasol_hookshots %>% #Create a new data set from the one above
  group_by(month_new) %>% #group by month 
  count(type) #count the number of hook shots recorded 
...
```{r}
# Your code here: 
storms_danger <- storms %>%  #Create a new data frame from the storms data set
  mutate(danger = case_when( #Create a new variable for danger
    status == "hurricane" ~ "high", #value is 'high' if the status is a hurricane
    status != "hurricane" ~ "moderate")) #value is 'moderate' if the status is not hurricane
